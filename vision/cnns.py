...
class AlexNet(nn.Module):
    def __init__(self, num_classes):
        ...
        # self.forward(torch.empty((64, 3, 224, 224)))
    ...
    def forward(self, imgs):
        ...
        return imgs_linear # because of batch_size
    def batch_loss(self, y_hat, y):
        # feed it in, get the outputs
        # across the entire batch, get the
        log_probs = torch.log(y_hat)
...
class NetworkInNetwork(nn.Module):
    ...
    def __init__(self, num_classes : int, mnist : bool = True):
        ...
        self.layers.append(nn.Flatten(start_dim=1)) # start_dim needs to be 1 for batch_size
        self.net = nn.Sequential(
            *self.layers
        )
...
class InceptionBlock(nn.Module):
    ...
    def __init__(self, c0, c1, c2, c3, **kwargs): # use the channels of each of the four parallel branches
        ...
        self.branch4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1), # need to put the stride=1 explicitly, otherwise it will be kernel_size and then shrink the image too much
            nn.LazyConv2d(c3, kernel_size=1),
            nn.ReLU()
        )
    def forward(self, imgs):
        # with respect to batching, we will cat along dim=1
        b1 = self.branch1(imgs)
        b2 = self.branch2(imgs)
        b3 = self.branch3(imgs)
        b4 = self.branch4(imgs)
        return torch.cat(
            (b1, b2, b3, b4), dim=1
        )
...
class BatchLeNet(nn.Module):
    ...
    def forward(self, img, train : bool):
        img = self.conv1(img)
        img = self.conv2(img)
        img = self.flatten(img)
        img = self.fc_layers(img)
...
class RegNet(nn.Module):
    def __init__(self, num_classes : int, using_mnist : bool, arch : tuple[int]):
        ...
        body_layers = []
        for stage in arch:
            body_layers.append(self.make_stage(*stage))
    ...
    def make_stage(self, out_channels : int, depth : int, groups : int, bottleneck : int):
        layers = []
        for ind in range(depth):
            if ind == 0:
                layers.append(ResNextBlock(mid_channels=out_channels // bottleneck, out_channels=out_channels,
                                        groups=groups, strides=2, use_channel=True)) # need a downsampler with stride 2 in the first one of each stage
            else:
                layers.append(ResNextBlock(mid_channels=out_channels // bottleneck, out_channels=out_channels,
                                         groups=groups, stides=1, use_channel=False))
